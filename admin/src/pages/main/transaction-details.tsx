...
import { useTransaction, useTransactions } from "@/hooks";
...
export default function TransactionDetails() {
  ...
  return (
    <MainLayout
      ...
      subtitle={`View details for ${transaction.reference}`}
    >
                ...
                <div className="bg-background dark:bg-secondary rounded-lg border border-line p-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {transaction.type === "deposit" ? (
                      <>
                        <button
                          onClick={() => approveDeposit(transaction.id)}
                          disabled={isLoading}
                          className="flex items-center space-x-2 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-md hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors disabled:opacity-50"
                        >
                          <CheckCircle className="w-4 h-4" />
                          <span>Approve Deposit</span>
                        </button>
                        <button
                          onClick={() => rejectDeposit(transaction.id)}
                          disabled={isLoading}
                          className="flex items-center space-x-2 px-4 py-2 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 rounded-md hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors disabled:opacity-50"
                        >
